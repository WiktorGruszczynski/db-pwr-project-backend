...
def create_and_send_2fa_code(
    user_id: str, user_email: str, code_type: str = "EMAIL_VERIFICATION"
) -> None:
    code = generate_code()
        ...
        with conn.cursor() as cursor:
            ...
            if existing_user:
                if existing_user["is_enabled"]:
                    raise HTTPException(
                        status_code=400, detail="Użytkownik już istnieje"
                    )
                else:
...
def verify_user_registration(data: UserVerify2FA) -> None:
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute(
                "SELECT id, is_enabled FROM users_user WHERE email = %s", (data.email,)
            )
            ...
            if user["is_enabled"]:
                raise HTTPException(
                    status_code=400, detail="Użytkownik jest już aktywny"
                )
            ...
            cursor.execute(
                "UPDATE users_user SET is_enabled = TRUE WHERE id = %s", (user["id"],)
            )
            cursor.execute(
...
def authenticate_user(user: UserLogin) -> str:
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute(
                "SELECT id, password, is_enabled FROM users_user WHERE email = %s",
                (user.email,),
            )
            ...
            if not db_user["is_enabled"]:
                raise HTTPException(
                    status_code=403,
                    detail="Najpierw potwierdź swój adres e-mail (skorzystaj ponownie z formularza rejestracji by wymusić nowy kod).",
                )
...
def verify_and_reset_password(email: str, code: str, new_password: str) -> None:
    ...
    try:
        with conn.cursor() as cursor:
            ...
            if not valid_code:
                raise HTTPException(
                    status_code=400, detail="Nieprawidłowy lub wygasły kod"
                )
